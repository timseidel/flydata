...
# Define a reusable theme for consistent plot styling
...
busiest_routes_plot <- busiest_routes |>
  slice_max(number_of_flights, n = 10) |>
...
```{r satisfaction, fig.width=12, fig.height=5}

if (!is.null(flights$punctuality_sat)) {
  set.seed(123)
  sampled_flights <- flights |>
    group_by(carrier) |>
    sample_n(size = min(100, n()), replace = FALSE)

  plot_data <- sampled_flights |>
    filter(!carrier %in% c("WN", "YX", "G4")) |>
    pivot_longer(
      cols = c(punctuality_sat, comfort_sat, crew_sat),
      names_to = "dimension",
      values_to = "satisfaction"
    ) |>
    mutate(dimension = case_when(
      dimension == "punctuality_sat" ~ "Punctuality",
      dimension == "comfort_sat" ~ "Comfort",
      dimension == "crew_sat" ~ "Crew Service"
    ))

  delay_satisfaction_plot <- plot_data |>
    ggplot(aes(x = arr_delay, y = satisfaction, color = carrier)) +
    geom_point(alpha = 0.25, size = 1, position = position_jitter(width = 5, height = 0.2)) +
    geom_smooth(aes(group = 1), method = "loess", se = FALSE,
                color = "black", linewidth = 0.8) +
    labs(
      title = "Customer Satisfaction vs. Arrival Delay",
      subtitle = "Punctuality drops with delay; comfort and crew service depend on other factors",
  ...
  print(delay_satisfaction_plot)
}

```

### Conclusion
The three satisfaction dimensions reveal different stories. Punctuality satisfaction drops sharply as delays increase — passengers clearly notice when flights are late. Comfort and crew service ratings, however, remain relatively stable regardless of delay. This suggests that comfort is more influenced by factors like aircraft age and seat configuration, while crew service depends on the airline itself rather than operational punctuality. Understanding these distinctions allows airlines to target their improvement efforts where they matter most for each aspect of the passenger experience.
